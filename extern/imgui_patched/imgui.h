...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_UndoRedo            = 1 << 16,  // Ensable undo/redo. Note that input text owns the text data while active, if you want to provide your own undo/redo stack you need e.g. to call ClearActiveID().
...
enum ImGuiDockNodeFlags_
{
    ...
    ImGuiDockNodeFlags_NoMove                       = 1 << 8,   // Shared/Local // tildearrow: Disable moving node (TODO: unnecessary with addition of NoUndocking?)
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_InertialScrollEnable   = 1 << 8,   // Docking enable flags.
    ImGuiConfigFlags_NoHoverColors          = 1 << 9,   // Disable all "hovered" color changes (useful for mobile).
...
enum ImGuiTableFlags_
{
    ...
    // tildearrow
    ImGuiTableFlags_NoBordersInFrozenArea      = 1 << 29,  // Disable vertical borders in frozen area.
    ImGuiTableFlags_NoScrollWithMouse          = 1 << 30,   // Disable user scrolling with mouse wheel. On child window, mouse wheel will be forwarded to the parent unless NoScrollbar is also set.
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddTextNoHashHide(ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL);
